...
@property (nonatomic, assign) RTSPStreamer *streamer;
...
- (id)initWithStreamer:(RTSPStreamer*)streamer {
    if (self = [super init]) {
...
//    }
    ...
    OSStatus status = AudioQueueNewOutput(&audioStreamBasicDesc_, audioQueueOutputCallback, (__bridge void*)self, nil, nil, 0, &audioQueue_);
    if (status != noErr) {
